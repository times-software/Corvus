\subsection{Getting Corvus}
To get corvus, go to 
\url{https://github.com/times-software/Corvus/releases/latest}
and download the archived .zip or .tgz file.

\subsubsection{Requirements}
Corvus is portable and runs on macOS, Linux, and Windos. 
To install corvus, you will need python. Versions > 3.12 and < 3.14 are supported. Earlier versions may work as well. We also strongly suggest that you use
an environment manager such as conda to install corvus. Our suggestion is to
use the free miniforge package, which can be found at \url{https://conda-forge.org/download/}

Python's venv environment management package can also be used.

To actually run corvus, you will need at least FEFF10. To get FEFF10, go to \url{https://times-software.github.io/feff10/} to register. Registration is free,
and is only used for communication about bugs and or updates, and for usage statistics. After registration, you will receive an email with directions for 
download and installation.

\subsection{Installation}

\subsubsection{Simple Installation using install script}
Note: The best way to install Corvus + FEFF is to install FEFF10 first. See instructions above. 

Once you have a version of Corvus from the github website, you can follow the following simple directions to install. Note that in the below "version" should be
replaced with the version that you downloaded, e.g., Corvus-3.2.0-01.zip.

\paragraph{MacOS} Unzip the downloaded file. Open a terminal and navigate to the file. Run the install.sh script.
\begin{verbatim}
cd Downloads
unzip Corvus-version.zip
cd Corvus-version/
chmod u+x install.sh
./install.sh
\end{verbatim}

\paragraph{Windows} Unzip the download file. Open a conda terminal (search for miniforge prompt in the searchbar if you installed miniforge) and navigate to the directory. Run the installer.bat script.
The following commands will do all of this, assuming you leave the zip file in your Downloads directory.
\begin{verbatim}
cd Downloads
unzip Corvus-version.zip
cd Corvus-version\
install.bat
\end{verbatim}
Note that if you unzipped using the explorer rather than the command-line "unzip" command, you will find that you need to 
\begin{verbatim}
cd Corvus-version\Corvus-version
\end{verbatim}

Once the installer finishes successfully, you can check the installation by activating the environment,
and running corvus.
\begin{verbatim}
conda activate Corvus
run-corvus
\end{verbatim}
When you do this,
Corvus will attempt to find FEFF10 in the standard FEFF10 installation directories. If corvus does not
find FEFF10, you will see a message stating such, and you will need to edit the configuration file.
This file is found in your home directory, 
\begin{verbatim}
Windows -- C:\Users\your_user_name\.Corvus\corvus.conf
MacOS -- /Users/your_user_name/.Corvus/corvus.conf
\end{verbatim}

Please edit this file and make sure it looks as follows:
\begin{verbatim}
[Executables]
dmdw     :
feff     : path_to_your_feff10_binaries_directory
abinit   :
nwchem   :
orca     :
gaussian :
vasp     :
siesta   :
ocean    :
cif2cell :
phsf     :

[Defaults]
prefix      : Corvus
inputsuffix : .inp
savesuffix  : .nest
checkpoints : off
parallelrun :
\end{verbatim}


\subsubsection{Step-by-step instructions with conda}
In MacOS, just open a terminal, and it will already have conda enabled if you have it installed. In windows, you will need to open a conda terminal, (Miniforge command window if you installed miniforge conda).
\begin{enumerate}
\item Download corvus from the github page.
\item unzip the folder.
\item In the terminal, navigate to the folder
\item Create a conda environment for corvus
\item Install corvus with pip.
\end{enumerate}
\begin{verbatim}
cd Downloads/Corvus-version/
conda create -name your_corvus_env python=">=3.12,<3.14"
conda activate your_corvus_env
pip install .
\end{verbatim}

If you would also like to install SciGUI (the GUI for Corvus), please go to https://github.com/times-software/SciGUI
and download and unzip the archived source, or use git to clone the repository. Now, open a terminal if you have closed it from the previous commands,
activate your\_corvus\_env. Navigate to the downloaded source folder and type,
\begin{verbatim}
pip install .
\end{verbatim}

Note that on linux, you will need to separately install the wxpython module.  
The pip version of wxpython is not reliable on all flavors of linux. The conda version seems to be more reliable, although this has 
not been tested extensively. 
\begin{verbatim}
conda install conda-forge::wxpython
\end{verbatim}
